...
float prev_locations[MAX_BOIDS][3];     // The previous location of the boid used for trails;
...
//Helper function to determin if the distance between two boids is greater than the r_rule
...
void WindowDisplay(void)
{
    ...
    glLoadIdentity();
    //Added glRotatef here so global_spinner would work
...
void updateBoid(int i)
{
    ...
    // needed.
    ///////////////////////////////////////////
    float velocity_pre_update[3];
    for (int z=0; z<3; z++){
        velocity_pre_update[z] = Boid_Velocity[i][z];
    }
    ...
    for (int b = 0; b<nBoids; b++){
        //check radius
        if ( i != b && check_in_radius(Boid_Location[i], Boid_Location[b], r_rule1)==1){
    ...
    if (r1_near_boids!=0){
        pc_j[0] =  pc_j[0] / r1_near_boids;
        pc_j[1] =  pc_j[1] / r1_near_boids;
        pc_j[2] =  pc_j[2] / r1_near_boids;
    }
    
    for (int z = 0; z < 3; z++){
        V1[z] = pc_j[z] - Boid_Location[i][z];
    }
    
    //Update velocity
    for (int x=0; x<3;x++){
        Boid_Velocity[i][x] +=  k_rule1 * V1[x];
    }
    ...
    while (k < nBoids){ 
        //pseudo: IF b != bJ THEN
        if (k!=i && check_in_radius(Boid_Location[i], Boid_Location[k], r_rule2)==1){
            for (int z=0;z<3;z++){
                V2[z] = Boid_Location[k][z] - Boid_Location[i][z];
                Boid_Velocity[i][z] -= k_rule2 * V2[z];
            }
        }
        k++;
    }
    ...
    if (r3_near_boids != 0){
        for (int z=0; z<3; z++){
            V3[z] =  pv_j[z]/r3_near_boids;
            Boid_Velocity[i][z] += k_rule3 * V3[z];        
        }
    }
    ...
    //rule 0 codes:
    for (int z=0; z<3; z++){
        Boid_Velocity[i][z] += k_rule0 * velocity_pre_update[z];
    }
    ...
    //Codes to record the previous location before update
    for (int z=0; z<3; z++){
        prev_locations[i][z] = Boid_Location[i][z];
    }
...
void drawBoid(int i)
{
    ...
    glColor4f(05,0,0.5,0.8);	// This specifies colour as R,G,B,alpha.
    ...
    //gluPartialDisk(my_quad,0.5,1,10,10,290,140);       // Draw this boid
    //glPopMatrix();
    
    //left wheel
    //glPushMatrix();
    //glTranslatef(Boid_Location[i][0],Boid_Location[i][1]-0.12,Boid_Location[i][2]+0.7);
    ...
    //glTranslatef(Boid_Location[i][0],Boid_Location[i][1]-0.5,Boid_Location[i][2]-0.7);  
    // gluDisk(my_quad,0.3,0.6,10,10);       // Draw this boid
    //glPopMatrix();         
    ...
    //Right wing
    glPushMatrix();
    glTranslatef(Boid_Location[i][0],Boid_Location[i][1]+2,Boid_Location[i][2]+0.7);  
    gluDisk(my_quad,0,1.5,10,10);       // Draw this boid
    glPopMatrix(); 
    
    //Left wing
    glPushMatrix();
    glTranslatef(Boid_Location[i][0],Boid_Location[i][1]+2,Boid_Location[i][2]+3);  
